$(function() {
  ...
  var db = new PouchDB('SEQdb');

  var SEQ = {}
  SEQ.sessionName = Date.now();
  console.log("SEQ.sessionName: " + SEQ.sessionName);
  Backbone.sync = BackbonePouch.sync();
    ...
    $('<svg version="1.1" style="display:none" id="new_seq_button" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="16px" height="16px" viewBox="8 8 16 16" enable-background="new 8 8 16 16" xml:space="preserve"><rect x="10.75" y="10.125" width="40.5" height="20.5" fill="#00ff00" /></svg>').appendTo('#loops');
    $('<svg version="1.1" style="display:none" id="play_seq_button" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="16px" height="16px" viewBox="8 8 16 16" enable-background="new 8 8 16 16" xml:space="preserve"><rect x="10.75" y="10.125" width="40.5" height="20.5" fill="blue" /></svg>').appendTo('#loops');
...
//            curpat.intervals.push(interval)
  ...
  $('#new_seq_button').bind('click', {}, function(event) {
    ...
    for (var beatInd = 0; beatInd < signature; beatInd++) {
      var event = beats['graphics'][beatInd]
      if (event != null) {
        var asset = event.asset;
        SEQ.currentBar = SEQ.currentBar + 1;
        beatAssets[beatInd] = asset
        event.clear()
      }
    }
    if (beatAssets.length > 0) {
      doc = {sessionName: SEQ.sessionName, currentBar: SEQ.currentBar, beatAssets:beatAssets};
